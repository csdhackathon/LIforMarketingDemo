...
public class MapUtility {
    ...
    public MapUtility(Activity activity, MapView mapView) {
        ...
        this.mUtility = Utils.getInstance(mActivity);
...
*/
    ...
    public void setMyLocationMarker(Location mLocation, boolean isAnimateToLocation, String address) {
        try {
            ...
            if(mMapView != null)  {
                ...
                if(mMarkerMyLocation != null) {
                    ...
                    Drawable mDrawable = ContextCompat.getDrawable(mActivity, R.drawable.ic_launcher);
                    mMarkerMyLocation.setIcon(mUtility.scaleImage(mActivity, mDrawable, (float) 0.65));
                    mMarkerMyLocation.setTitle(address);

                    MyLocationInfoWindow mMyLocationInfoWindow = new MyLocationInfoWindow(R.layout.layout_custom_info_window, mMapView, address);
                ...
                if(isAnimateToLocation) {
                    ...
                    //String myLocationTitle = address;
                    for(int i=0; i<mMapView.getOverlays().size(); ++i){
                        Overlay o = mMapView.getOverlays().get(i);
                        if(o instanceof Marker){
                            Marker m = (Marker) o;
                            if(m.getTitle() != null && m.getTitle().equalsIgnoreCase(address)) {
                                m.showInfoWindow();
